...
- **Results**: full-height burst strip left (CoverArtRect, ~420px — box aspect 0.583 vs art 0.5625 so virtually no crop) with a right-edge **alpha** fade (CoverArtRect.fade_right=0.25 → art alpha 100%→0% over the rightmost 25%, dissolving into the background — the old dark `ArtFadeH` scrim was removed so the right edge reaches true 0 opacity) + a bottom gradient fade (`ArtFadeV`) and NIKKE // name; right column = verdict (MISSION COMPLETE green / NIKKE DOWN red, themed subtitle), yellow FINAL SCORE + skewed ☠ GODDESS FALL chip on one baseline, 3-col report cells (wave/time/difficulty/kills/bosses/damage dealt), then **reward bar on victory / damage log on defeat** (user: defeat only), commands pinned bottom (PLAY AGAIN / RETRY MISSION primary).
