...
var kilo_burst_duration = 5.0  # Base duration, upgraded to 10s
...
func _activate_kilo_burst():
	# Kilo burst: Assign Priority - infinite ammo, rapid fire, 2.2x damage
	kilo_burst_active = true
	
	# Check for duration upgrade (10s instead of 5s)
	var duration_level := get_talent_level(3, "burst_duration")
	kilo_burst_duration = 10.0 if duration_level > 0 else 5.0
	kilo_burst_timer = kilo_burst_duration
	
	# Check for invincibility upgrade
	var invuln_level := get_talent_level(3, "burst_invuln")
	if invuln_level > 0:
