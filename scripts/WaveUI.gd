...
const BOSS_WARNING_DURATION := 2.0
...
func _setup_ui() -> void:
	...
	_boss_warning = preload("res://scripts/WarningBar.gd").new()
...
func update_time(elapsed: float, _remaining: float) -> void:
	if _timer_label:
...
func show_event(event_type: String, _event_data: Dictionary) -> void:
	if not _event_label:
		return
