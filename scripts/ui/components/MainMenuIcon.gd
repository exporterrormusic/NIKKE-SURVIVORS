...
func _draw_characters(center: Vector2, icon_size: float, color: Color) -> void:
	# Draw a stylized group of people icon
	var head_radius := icon_size * 0.12
	var body_height := icon_size * 0.25
	
	# Center person (slightly larger)
	draw_circle(center + Vector2(0, -icon_size * 0.15), head_radius * 1.1, color)
	var body_top := center + Vector2(0, -icon_size * 0.05)
	var body_bottom := center + Vector2(0, body_height * 0.8)
	draw_line(body_top, body_bottom, color, 4.0)
	
	# Left person
	var left_offset := Vector2(-icon_size * 0.22, icon_size * 0.05)
	draw_circle(center + left_offset + Vector2(0, -icon_size * 0.12), head_radius * 0.9, color)
	draw_line(center + left_offset, center + left_offset + Vector2(0, body_height * 0.6), color, 3.0)
	
	# Right person
	var right_offset := Vector2(icon_size * 0.22, icon_size * 0.05)
	draw_circle(center + right_offset + Vector2(0, -icon_size * 0.12), head_radius * 0.9, color)
	draw_line(center + right_offset, center + right_offset + Vector2(0, body_height * 0.6), color, 3.0)
