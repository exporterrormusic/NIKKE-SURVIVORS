...
func setup(projectile: Node, bounce_visuals_enabled: bool) -> void:
	...
	if shape_key == "" and projectile.has_method("get") and projectile.get("shape") != null:
		shape_key = String(projectile.get("shape")).to_lower()
...
func _compute_laser_dimensions(radius: float) -> Dictionary:
	var archetype := ""
	var projectile_range := 0.0
	if _projectile and is_instance_valid(_projectile):
		var archetype_value: Variant = _projectile.get("projectile_archetype")
		if archetype_value is String:
			archetype = (archetype_value as String).to_lower()
		var range_value: Variant = _projectile.get("max_range")
		if range_value is float or range_value is int:
			projectile_range = float(range_value)
	var is_enemy := archetype == "enemy_laser"
	var use_special := _is_special_attack() and not is_enemy
	var length_multiplier: float = 120.0 if use_special else 115.0
	var min_length: float = 360.0 if use_special else 420.0
	var width_multiplier: float = 6.0 if use_special else 4.0
	var min_width: float = 14.0 if use_special else 10.0
	var neck_factor: float = 0.16 if use_special else 0.1
	var neck_max: float = 48.0 if use_special else 64.0
	var neck_min: float = 12.0
	var tip_factor: float = 0.12 if use_special else 0.08
	var tip_max: float = 48.0 if use_special else 72.0
	var tip_min: float = 18.0
	if is_enemy:
		length_multiplier = 20.0
		min_length = 40.0
		width_multiplier = 6.25
		min_width = 5.0
		neck_factor = 0.2
		neck_max = 15.0
		neck_min = 7.5
	...
	if projectile_range > 0.0:
		beam_length = min(beam_length, projectile_range)
	var beam_width: float = max(radius * width_multiplier, min_width)
	return {
...
func _update_laser_body(direction: Vector2, radius: float, color: Color) -> void:
	...
	if dir == Vector2.ZERO:
		dir = Vector2.RIGHT
	var dims := _compute_laser_dimensions(radius)
	var beam_length: float = dims.get("beam_length", 420.0)
	var beam_width: float = dims.get("beam_width", 10.0)
	var neck_factor: float = dims.get("neck_factor", 0.1)
	var neck_max: float = dims.get("neck_max", 64.0)
	var neck_min: float = dims.get("neck_min", 12.0)
	var tip_factor: float = dims.get("tip_factor", 0.08)
	var tip_max: float = dims.get("tip_max", 72.0)
	var tip_min: float = dims.get("tip_min", 18.0)
	var neck_back: float = clampf(beam_length * neck_factor, neck_min, neck_max)
	var tip_length: float = clampf(beam_length * tip_factor, tip_min, tip_max)
	var layer_widths: Array = [beam_width * 3.0, beam_width * 2.0, beam_width * 1.3]
	var use_special: bool = dims.get("use_special", false)
	var layer_colors: Array = _resolve_laser_layer_colors(color, use_special)
