...
class_name ShotgunPelletVisual
...
func _ready() -> void:
	if bullet_sprite:
		...
		shader_material.set_shader_parameter("glow_color", Color(1.0, 0.5, 0.0, 1.0))
...
func update_visual(direction: Vector2, radius: float, color: Color, _context: Dictionary = {}) -> void:
	...
	if bullet_sprite:
		...
		var glow_intensity: float = 0.3 if is_day_scene else 0.8
		var glow_size: float = 0.004 if is_day_scene else 0.012
